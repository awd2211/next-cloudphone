...
// 获取审计日志详情
export const getAuditLogDetail = (id: string): Promise<AuditLog> =>
...
export const getUserAuditLogs = (
  ...
  api.get<PaginatedResponse<AuditLog>>(`/audit-logs/user/${userId}`, { params });
...
export const exportAuditLogs = (params?: AuditLogFilter): Promise<Blob> =>
  request.get('/audit-logs/export', {
    params,
    responseType: 'blob',
  });

// 获取审计日志统计
export const getAuditStats = (params?: { startDate?: string; endDate?: string }): Promise<{
  totalLogs: number;
  successCount: number;
  failedCount: number;
  warningCount: number;
...
// 清理旧日志（管理员功能）
export const cleanOldAuditLogs = (beforeDate: string): Promise<void> =>
  api.delete<void>('/audit-logs/cleanup', {
    data: { beforeDate },
  });
