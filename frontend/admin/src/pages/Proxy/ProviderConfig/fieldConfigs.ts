...
export const providerFieldsConfig: ProviderFieldsConfig = {
  ...
  // ==================== SmartProxy (Decodo) 配置 ====================
  // 官方文档: https://help.decodo.com/docs/residential-proxy-quick-start
  // 代理端点: gate.decodo.com:7000
  // 用户名格式: user-{username}-country-{cc}-state-{state}-city-{city}-session-{id}-sessionduration-{min}
  ...
  smartproxy: [
    {
      ...
      required: true,
      placeholder: 'your-username',
      tooltip: 'SmartProxy/Decodo 代理用户名（系统会自动添加 user- 前缀）',
    },
    {
      name: 'password',
      label: '密码',
      ...
      required: true,
      placeholder: 'your-password',
      tooltip: 'SmartProxy/Decodo 代理密码',
    },
    // ========== 高级参数 (地理位置定位) ==========
    {
      name: 'defaultContinent',
      label: '默认大洲',
      type: 'select',
      required: false,
      tooltip: '按大洲筛选代理IP',
      options: [
        ...
        { label: '🌎 南美 (SA)', value: 'sa' },
        { label: '🌍 非洲 (AF)', value: 'af' },
    ...
    },
    {
      ...
      required: false,
      placeholder: 'california',
      tooltip: '仅限美国，州名全拼小写。格式: 州名 (如 california, new_york)。系统会自动添加 us_ 前缀',
    },
    {
      name: 'defaultCity',
      label: '默认城市',
      ...
      tooltip: '城市名，多词用下划线连接 (如 new_york, los_angeles, las_vegas)',
    },
    {
      name: 'defaultZip',
      label: '默认邮编 (仅限美国)',
    ...
    },
    {
      ...
      required: false,
      placeholder: '7922',
      tooltip: 'ASN 运营商号码。注意：ASN 定位与城市定位不能同时使用',
    },
    // ========== 会话控制 ==========
    {
      name: 'sessionMode',
      ...
      required: false,
      defaultValue: 'rotating',
      tooltip: 'rotating: 每次请求换IP；sticky: 保持同一IP直到会话过期',
      options: [
        { label: '轮换模式 (Rotating) - 每次请求换IP', value: 'rotating' },
        { label: '粘性会话 (Sticky) - 保持同一IP', value: 'sticky' },
      ],
    },
    {
      ...
      defaultValue: 10,
      placeholder: '1-1440',
      tooltip: '粘性会话模式下IP保持时间。范围: 1-1440 分钟（默认10分钟，最长24小时）。注意：会话时长越长，IP因设备离线而提前轮换的概率越高',
      min: 1,
      max: 1440,
    },
  ],
