...
export const ThrottledScrollContainer = memo(
  ({ children, onScroll, throttleDelay = 200, className }: ThrottledScrollContainerProps) => {
...
export const DelayedRender = memo(
  ({ children, delay = 1000, placeholder = null }: DelayedRenderProps) => {
  ...
  return (
    <div>
      ...
      {items.map((item) => {
        ...
        return <div key={id}>{renderItem(item, selected, () => toggleItem(id))}</div>;
      })}
    </div>
  );
}
