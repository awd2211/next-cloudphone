...
describe('DockerService', () => {
  ...
  describe('initialization', () => {
    ...
    it('should use custom docker host from config', () => {
      ...
      // Act
      const newService = new DockerService(mockConfigService as any, mockGpuManager as any);
  ...
  describe('createContainer', () => {
    ...
    it('should successfully create a container with basic config', async () => {
      ...
      mockDockerClient.modem.followProgress.mockImplementation((stream, onFinish) => {
        onFinish(null, []);
      });
      ...
      expect(mockDockerClient.createContainer).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          Env: expect.arrayContaining(['WIDTH=1080', 'HEIGHT=1920', 'DPI=320', 'REDROID_AUDIO=1']),
        })
      );
    });
    ...
    it('should create container with GPU support when enabled', async () => {
      ...
      mockGpuManager.getGpuEnvironment.mockReturnValue(['NVIDIA_VISIBLE_DEVICES=all']);
      ...
      // Assert
      expect(result).toBe(mockContainer);
      expect(mockGpuManager.getRecommendedConfig).toHaveBeenCalledWith('balanced');
  ...
  describe('startContainer', () => {
    ...
    it('should handle start failure', async () => {
      ...
      // Act & Assert
      await expect(service.startContainer(containerId)).rejects.toThrow('Container start failed');
    });
  });
  ...
  describe('stopContainer', () => {
    ...
    it('should throw DockerError when stop fails', async () => {
      ...
      await expect(service.stopContainer(containerId)).rejects.toThrow('Failed to stop container');
    });
  });
  ...
  describe('removeContainer', () => {
    ...
    it('should throw error when remove fails', async () => {
      ...
      // Act & Assert
      await expect(service.removeContainer(containerId)).rejects.toThrow('Remove failed');
    });
  });
  ...
  describe('getContainerInfo', () => {
    ...
    it('should handle container not found', async () => {
      ...
      // Act & Assert
      await expect(service.getContainerInfo(containerId)).rejects.toThrow('No such container');
    });
  });
