...
export async function seedDevices(dataSource: DataSource, userIds: string[]) {
  ...
  for (const nodeData of nodes) {
    const existing = await nodeRepository.findOne({ where: { name: nodeData.name } });
    if (!existing) {
  ...
  for (const templateData of templates) {
    const existing = await templateRepository.findOne({ where: { name: templateData.name } });
    if (!existing) {
  ...
  return { devices: createdDevices, templates: createdTemplates, nodes: createdNodes };
}
