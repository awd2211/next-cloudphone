...
export class DevicesConsumer {
  ...
  async handleAppInstall(event: AppInstallRequestedEvent) {
    ...
    try {
      ...
      const apkPath = await this.downloadApkSecure(event.downloadUrl, event.appId, checksum);
      ...
      this.logger.log(`App ${event.appId} installed successfully on device ${event.deviceId}`);
    } catch (error) {
      this.logger.error(
    ...
    try {
      ...
      if (parsedUrl.protocol !== 'https:') {
        if (
          ...
          this.logger.warn(`Allowing HTTP download from localhost: ${url} (development only)`);
        } else {
          throw new BadRequestException('只允许 HTTPS URL 或 localhost HTTP URL');
        }
      }
      ...
      if (!isAllowedDomain) {
        ...
        throw new BadRequestException(`不允许从该域名下载: ${parsedUrl.hostname}`);
      }
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        request.on('response', (response) => {
          ...
          if (response.statusCode !== 200) {
            reject(new BadRequestException(`下载失败: HTTP ${response.statusCode}`));
            return;
          }
          ...
          if (contentLength > MAX_APK_SIZE) {
            reject(new BadRequestException(`APK 文件过大: ${contentLength} bytes`));
            return;
          }
          ...
          writeStream.on('finish', async () => {
            try {
              ...
              if (expectedChecksum) {
                ...
                this.logger.warn('No checksum provided, skipping verification (not recommended)');
              }
