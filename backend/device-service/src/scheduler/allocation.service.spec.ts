...
describe('AllocationService', () => {
  ...
  describe('allocateDevice', () => {
    ...
    beforeEach(() => {
      ...
      jest
        ...
        .mockReturnValue(mockAllocation as DeviceAllocation);
      jest
        .spyOn(allocationRepository, 'save')
        .mockResolvedValue(mockAllocation as DeviceAllocation);
    ...
    it('should select device based on preferred specs', async () => {
      ...
      jest
        .spyOn(service, 'getAvailableDevices')
        .mockResolvedValue([mockDevice as Device, highSpecDevice as Device]);
    ...
    it('should handle quota service unavailable gracefully', async () => {
      jest
        .spyOn(quotaClient, 'checkDeviceCreationQuota')
  ...
  describe('releaseDevice', () => {
    ...
    it('should successfully release a device', async () => {
      ...
      expect(allocationRepository.save).toHaveBeenCalled();
      expect(quotaClient.reportDeviceUsage).toHaveBeenCalledWith(
        userId,
        expect.objectContaining({
          deviceId,
          operation: 'decrement',
    ...
    it('should publish failed billing data to DLX when billing fails', async () => {
      jest
        .spyOn(billingClient, 'reportDeviceUsage')
        .mockRejectedValue(new Error('Billing service unavailable'));
  ...
  describe('getAvailableDevices', () => {
    ...
    it('should filter out allocated devices', async () => {
      jest
        ...
        .mockResolvedValue([mockDevice as Device, { ...mockDevice, id: 'device-2' } as Device]);
      jest
  ...
  describe('releaseExpiredAllocations', () => {
    it('should release expired allocations', async () => {
      ...
      jest
        .spyOn(allocationRepository, 'save')
        .mockResolvedValue(expiredAllocation as DeviceAllocation);
  ...
  describe('getAllocationStats', () => {
    it('should return allocation statistics', async () => {
      jest
        .spyOn(allocationRepository, 'count')
  ...
  describe('extendAllocation', () => {
    ...
    it('should throw BadRequestException when allocation not active', async () => {
      ...
      await expect(service.extendAllocation('allocation-1', 30)).rejects.toThrow(
        BadRequestException
      );
    });
    ...
    it('should enforce maximum extend minutes per request', async () => {
      ...
      await expect(service.extendAllocation('allocation-1', 200)).rejects.toThrow(
        BadRequestException
      );
    });
