 ...
 * 阿里云ECP配置
 */
export interface AliyunEcpV2Config {
  accessKeyId: string;
...
@Injectable()
export class AliyunEcpV2Client implements OnModuleInit {
  private readonly logger = new Logger(AliyunEcpV2Client.name);
  private config: AliyunEcpV2Config;
  ...
  private async initializeClient(): Promise<void> {
    ...
    try {
      ...
      this.logger.log(`AliyunEcpV2Client initialized for region: ${this.config.regionId}`);
    } catch (error) {
