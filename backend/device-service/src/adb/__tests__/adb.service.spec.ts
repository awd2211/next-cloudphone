...
describe('AdbService', () => {
  ...
  describe('disconnectFromDevice', () => {
    ...
    it('should not throw error if device is not connected', async () => {
      await expect(service.disconnectFromDevice('nonexistent-device')).resolves.not.toThrow();
    });
  });
  ...
  describe('installApk', () => {
    ...
    it('should successfully install APK', async () => {
      ...
      expect(mockAdbClient.install).toHaveBeenCalledWith('localhost:5555', '/path/to/app.apk', {
        reinstall: false,
      });
    });
  ...
  describe('uninstallApp', () => {
    ...
    it('should successfully uninstall app', async () => {
      ...
      const result = await service.uninstallApp('device-123', 'com.example.app');
