...
export class BackupExpirationService {
    ...
    try {
      ...
      for (const snapshot of oldBackups) {
        try {
          ...
          this.logger.warn(
            `Failed to delete old backup ${snapshot.id}: ${error.message}`,
          );
        }
      }
        ...
        if (snapshots.length > this.config.maxBackupsPerDevice) {
          ...
          for (const snapshot of toDelete) {
            try {
              ...
              cleanedCount++;
              this.logger.log(
                `Deleted excess backup for device ${device.id}: ${snapshot.id}`,
              );
            } catch (error) {
              this.logger.warn(
                `Failed to delete excess backup ${snapshot.id}: ${error.message}`,
              );
            }
          }
        }
   ...
   * 清理已过期的快照
   */
  private async cleanupExpiredSnapshots(
    ...
    for (const snapshot of snapshots) {
      try {
        ...
        this.logger.warn(
          `Failed to delete expired snapshot ${snapshot.id}: ${error.message}`,
        );
      }
    }
  }
