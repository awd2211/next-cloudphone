...
export class AutoScalingService {
  ...
  private loadConfig(): void {
    this.config = {
      ...
      targetCpuUtilization: +this.configService.get(
        'AUTOSCALING_TARGET_CPU',
        70,
      ),
      targetMemoryUtilization: +this.configService.get(
        'AUTOSCALING_TARGET_MEMORY',
        70,
      ),
      scaleUpThreshold: +this.configService.get(
        'AUTOSCALING_SCALE_UP_THRESHOLD',
        80,
      ),
      scaleDownThreshold: +this.configService.get(
        'AUTOSCALING_SCALE_DOWN_THRESHOLD',
        30,
      ),
      cooldownMinutes: +this.configService.get(
        'AUTOSCALING_COOLDOWN_MINUTES',
        10,
      ),
      scaleUpIncrement: +this.configService.get(
        'AUTOSCALING_SCALE_UP_INCREMENT',
        2,
      ),
      scaleDownDecrement: +this.configService.get(
        'AUTOSCALING_SCALE_DOWN_DECREMENT',
        1,
      ),
    };

    this.logger.log(
      `自动扩缩容配置已加载: ${this.config.enabled ? '已启用' : '已禁用'}`,
    );
  ...
  private async collectMetrics(): Promise<{
    ...
    for (const device of runningDevices) {
      try {
        ...
        this.logger.warn(
          `获取设备 ${device.id} 资源使用率失败: ${error.message}`,
        );
      }
    }
