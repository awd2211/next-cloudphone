...
export const getDatabaseConfig = (configService: ConfigService): TypeOrmModuleOptions => {
  ...
  console.log('========================================');
  console.log(`服务: billing-service`);
  console.log(`数据库: cloudphone_billing`);
  ...
  console.log(`特殊优化: 聚合查询加速（慢查询阈值 500ms）`);
  console.log('========================================');
  ...
  return {
    ...
    extra: {
      ...
      // 应用名称，方便在数据库端识别连接来源
      application_name: configService.get('DB_APPLICATION_NAME', 'billing-service'),
      ...
      // 对于 billing-service 的聚合查询，性能提升可达 40-50%
    ...
    // 慢查询日志配置
    // ========================================================================
    // billing-service 特殊优化：更严格的慢查询阈值
    maxQueryExecutionTime: parseInt(
      configService.get('DB_SLOW_QUERY_THRESHOLD', isProduction ? '500' : '5000'),
      10
    ), // 生产环境 500ms（更严格）
...
export const connectionPoolHealthConfig = {
  ...
  // 慢查询告警阈值（毫秒）- billing-service 更严格
  SLOW_QUERY_WARNING_THRESHOLD: 500, // 500ms
  SLOW_QUERY_CRITICAL_THRESHOLD: 2000, // 2s
};
