...
export class AccountUnlockedEventHandler implements IEventHandler<AccountUnlockedEvent> {
  ...
  async handle(event: AccountUnlockedEvent) {
    try {
      this.logger.log(`Handling AccountUnlockedEvent for user: ${event.aggregateId}`);

      // 事件已经在 EventStoreService 中保存和发布
      // 这里可以添加其他副作用，例如：

      // 1. 记录解锁日志
      this.logger.debug(`Account unlocked for user ${event.aggregateId} by ${event.unlockedBy}`);
      ...
      this.logger.log(`AccountUnlockedEvent handled successfully for user: ${event.aggregateId}`);
    } catch (error) {
      this.logger.error(
