...
export const hashSync = jest.fn((password: string, rounds: number) =>
  `$2b$${rounds}$hashed_${password}`
);

export const compareSync = jest.fn((password: string, hash: string) =>
  password === 'correct-password' || password === 'OldPass123!'
);

export const genSaltSync = jest.fn((rounds: number) =>
  `$2b$${rounds}$saltsaltsaltsaltsalt`
);
