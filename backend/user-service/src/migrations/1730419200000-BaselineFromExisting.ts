 ...
 * - departments, tenants, data_scopes, field_permissions
 * - quotas, api_keys, audit_logs, tickets
 * - user_events (分区表), user_snapshots
 * - settings, menus
...
export class BaselineFromExisting1730419200000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    // 空实现 - 所有表已存在于数据库
    // 这个迁移只是标记当前状态为基线
    console.log('✅ [User Service] Baseline migration - 所有表已存在');
    console.log('📊 当前数据库包含 17 张表');
  }
