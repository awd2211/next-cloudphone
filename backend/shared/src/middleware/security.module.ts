...
export class SecurityModule implements NestModule {
  configure(consumer: MiddlewareConsumer) {
    ...
    // 2. IP 黑名单 - 尽早拦截恶意 IP
    consumer.apply(IPBlacklistMiddleware).forRoutes({ path: '*', method: RequestMethod.ALL });
    ...
    // 4. XSS 防护 - 清理请求输入
    consumer.apply(XssProtectionMiddleware).forRoutes({ path: '*', method: RequestMethod.ALL });
    ...
    // 6. 自动封禁 - 监控并自动封禁异常行为
    consumer.apply(AutoBanMiddleware).forRoutes({ path: '*', method: RequestMethod.ALL });
  }
}
