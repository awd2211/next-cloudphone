...
async function main() {
  ...
  try {
    ...
    console.log(`✅ 登录: 成功`);
    console.log(`${gatewayDataScopes.data?.length > 0 ? '✅' : '❌'} API Gateway - data-scopes: ${gatewayDataScopes.data?.length || 0} 条`);
    console.log(`${gatewayFieldPerm.data?.length > 0 ? '✅' : '❌'} API Gateway - field-permissions: ${gatewayFieldPerm.data?.length || 0} 条`);
    console.log(`✅ User Service - data-scopes (直连): ${directDataScopes.data?.length || 0} 条`);
    console.log(`${directFieldPerm.data?.length > 0 ? '✅' : '⚠️'} User Service - field-permissions (直连): ${directFieldPerm.data?.length || 0} 条`);
    console.log('='.repeat(60));
